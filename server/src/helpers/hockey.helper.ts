    ...
    async getGames(teams: string[]): Promise<Game[]> {
        ...
        const teamPromises = teams.map(async team => {
            try {
                ...
                const response = await axios.get(`https://api-web.nhle.com/v1/club-schedule-season/${team}/20232024`);
                response.data.games.forEach((element: any) => {
                    if (!teamGameList.some(game => game.id === element.id)) {
                        const newGame = new Game(
                            ...
                            (element.gameState === 'OFF' && element.awayTeam.score === 0 && element.homeTeam.score === 0) || (element.gameState === 'ON') 
                                ? result.INCOMPLETE 
